...
async def upload_pdf_route(
    ...
    """
    Rota para upload de PDF. Somente usuários 'gerente' podem enviar.
    AGORA: Apenas faz upload e extração de texto, SEM geração de embeddings.
    """
    try:
        # Validação do role
        if user.get("role") != "gerente":
        ...
        # 2. Processa o PDF (apenas extrai texto e salva no banco)
        print(f"[DEBUG] Iniciando extração de texto do PDF: {file_display_name}")
        ...
        # Atualiza o registro com o texto extraído
        update_payload = {
            "status": "processado",  # Muda de "pendente" para "processado"
            "full_text": full_text,
            "processed_at": meta.get("downloaded_at"),
        }
        ...
        # 3. REMOVIDO: Geração de embeddings (será feito via rota separada)
        ...
        return {
            **result,
            "processing": {
                "text_extraction": "sucesso",
                "pages": meta.get("pages", 0),
                "embedding_generation": "pendente",  # Agora é pendente
            },
        }
