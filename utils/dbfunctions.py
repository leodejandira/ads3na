...
def listar_registros() -> List[Registro]:
    ...
    Returns:
        List[Registro]: Lista de objetos "Registro". Retorna lista vazia caso não tenha registros.
    ...
    try:
        ...
        raise HTTPException(status_code=500,
                            detail=f"Erro ao listrar registros: {str(e)}")
...
def inserir_registro(data: RegistroCreate) -> Registro:
    ...
    Notes:
        - A senha é truncada para evitar problemas com o limite do bcrypt (72 bytes).
    """
    try:
        supabase = get_client()
...
def buscar_por_email(email: str):
    ...
    Retorno:
        dict | None: Dicionário com os dados do usuário caso encontrado, ou None
        se não exister
    """
    supabase = get_client()
